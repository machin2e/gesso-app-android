package camp.computer.clay.application.storage;
...
import camp.computer.clay.old_model.Action;
import camp.computer.clay.old_model.Event;
import camp.computer.clay.old_model.PhoneHost;
import camp.computer.clay.old_model.Script;
import camp.computer.clay.old_model.State;
import camp.computer.clay.old_model.Timeline;

public class SQLiteStoreHost {
...
//            behavior.setScript(behaviorScript);
        ...
        public void insertDevice(PhoneHost phoneHost) {
            ...
            // Create a new map of values, where column names are the keys
            ContentValues values = new ContentValues();
            values.put(DeviceEntry.COLUMN_NAME_UUID, phoneHost.getUuid().toString());
            values.put(DeviceEntry.COLUMN_NAME_TIMELINE_UUID, phoneHost.getTimeline().getUuid().toString());

            // Insert the new row, returning the primary key value of the new row
            long entryId = db.insert(DeviceEntry.TABLE_NAME, null, values);

            Log.v("Content_Manager", "Inserted phoneHost into database (_id: " + entryId + ")");

        }

        public PhoneHost queryUnit(UUID uuid) {
            ...
            while (!cursor.isAfterLast()) {
                ...
                // Create the behavior
                PhoneHost phoneHost = new PhoneHost(getClay(), UUID.fromString(uuidString));
                phoneHost.getTimeline().setUuid(UUID.fromString(timelineUuidString));

                // Reconstruct the phoneHost's timeline
                queryTimeline(phoneHost, UUID.fromString(timelineUuidString));
        ...
        public Timeline queryTimeline(PhoneHost phoneHost, UUID timelineUuid) {
            ...
            queryEvents(phoneHost, timeline);

            // Assign the timeline to the phoneHost
            phoneHost.setTimeline(timeline);
...
//                    action.setUuid (existingBehaviorUuid);
         ...
         * @param timeline
         */
        public void queryEvents(PhoneHost phoneHost, Timeline timeline) {
...
//        }
    ...
    public Timeline restoreTimeline(PhoneHost phoneHost, UUID uuid) {
        Timeline timeline = db.queryTimeline(phoneHost, uuid);
        return timeline;
    }
    ...
    public void storeDevice(PhoneHost phoneHost) {
        db.insertDevice(phoneHost);
    }

    public PhoneHost restoreDevice(UUID uuid) {
        PhoneHost phoneHost = db.queryUnit(uuid);
        return phoneHost;
    }
    ...
    public void writeDatabase() {
        try {
            db.writeDatabaseToSD(SQLiteStoreHost.DATABASE_NAME);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
