...
public class Clay {
    ...
    // Group of discovered touchscreen hosts
    ...
    public Clay() {
        ...
        // Descriptor
        // TODO: Stream this in from the Internet and hosts.
        descriptor = new Descriptor("clay", "");
        descriptor.list("hosts");
...
//        setupSimulation();
        ...
        Launcher.getLauncherView().getDisplay().setScene(scene);
...
//        // </FORM_CONFIGURATION>
//
//        // TODO: Move Model/Host this into Model or _Ecology (in Model) --- maybe combine Model+_Ecology
...
//    }
    ...
    private void simulateHost() {
        ...
        camp.computer.clay.model.architecture.Host host = new camp.computer.clay.model.architecture.Host();
...
//        Log.v("UDP", "found host");
        ...
        // Search for the host in the store
        if (hasDeviceByUuid(deviceUuid)) {
            return null;
        }

        // Try to restore the host profile from the storeHost.
        Host host = getStore().restoreDevice(deviceUuid);
        ...
        */
        ...
        // Update the host's profile based on information received from host itself.
        if (host != null) {

            // Data.
            Descriptor deviceDescriptor = getClay().getDescriptor().get("hosts").put(deviceUuid.toString());
...
//                ApplicationView.getLauncherView().mapView.getFeature().simulateHost(new Host());

                // Add timelines to attached displays
                for (DisplayHostInterface view : this.displays) {
                    view.addDeviceView(host);
                }
            }

            Log.v("TCP", "host.internetAddress (3): " + internetAddress);

            // Establish TCP connection
            host.connectTcp();

            Log.v("TCP", "host.internetAddress (4): " + internetAddress);
            ...
            if (isNew) {
                ...
                // <HACK>
                host.enqueueMessage("request reset");
